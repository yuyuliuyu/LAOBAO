...
public class SalaryInsuranceServiceImpl extends BaseServiceImpl<SalaryInsurance, String>
    ...
    public Map<String, Object> getPaymentInsuranceListData(String depId, String searchData, Pager pager) {
        return salaryInsuranceDao.getPaymentInsuranceListData(depId, searchData, pager);
    }
    ...
    public String saveOrUpdateToInsurance(String formdata, String griddata) throws Exception {
        ...
        for (int i = 0, l = staffList.size(); i < l; i++) {
            ...
            jc.put("B_staffId", staffMap.get("id"));
            jc.put("B_companyId", staffMap.get("comId"));
            ...
            for (int j = 0, n = list.size(); j < n; j++) {
                ...
                jc.put("insuranceId", items.getIbfId());

                // 计算缴纳费用
                // 公司缴费基数为空
                // 页面缴费基数为空，则按照公式计算缴费基数和缴纳费用，存在缴费基数则缴费基数*保险的缴费比例
                Object comBase = "0";
//                Double comJn = 0d;
                SalaryInsurance ins = get(SalaryInsurance.class,
                        Restrictions.eq("companyId", staffMap.get("comId")),
                        Restrictions.eq("name", map.get("ibfId")),
                        Restrictions.eq("isDelete", 0));
                if (ins == null) {
                    throw new Exception("第"+(j+1)+"个福利项目不存在");
                }
                if (StringUtils.isBlank(items.getBaseCompany())) {
                    // 按照保险缴费的缴费基数公式计算
                    // 公司缴费公式
                    if (!StringUtils.isBlank(ins.getBaseCompany())) {
                        comBase = salaryFormulaService.formula(ins.getBaseCompanyMath(), jc);
//                        comJn = Double.valueOf(salaryFormulaService.formula(ins.getCompanyFormulaMath(), jc).toString());
                    }
                } else {
                    comBase = map.get("basePersonal");
//                    comJn = Double.valueOf(salaryFormulaService.formula(ins.getBiCompany(), jc).toString()) * Double.valueOf(items.getBaseCompany());
                }
                // 员工缴费基数为空
                Object perBase = "0";
//                Double perJn = 0d;
                if (StringUtils.isBlank(items.getBasePersonal())) {
                    // 按照保险缴费的缴费基数公式计算
                    // 员工缴费公式
                    if (!StringUtils.isBlank(ins.getBasePersonal())) {
                        perBase = salaryFormulaService.formula(ins.getBasePersonalMath(), jc);
//                        perJn = Double.valueOf(salaryFormulaService.formula(ins.getPersonalFormulaMath(), jc).toString());
                    }
                } else {
                    comBase = map.get("baseCompany");
//                    perJn = Double.valueOf(salaryFormulaService.formula(ins.getBiPersonal(), jc).toString()) * Double.valueOf(items.getBaseCompany());
                }

                Double bseCom = Double.valueOf(comBase.toString());
                Double bsePer = Double.valueOf(perBase.toString());
                if (bseCom > Double.valueOf(map.get("high"))) {// 公司基数缴纳上限比较
                ...
                // 员工不交费
                if (Integer.valueOf(map.get("isPerson")) == 0) {
//                    perJn = 0d;
                    bsePer = 0d;
                }
                items.setBaseCompany(bseCom.toString());
                items.setBasePersonal(bsePer.toString());
                items.setPaymentCompany("0");
                items.setPaymentPersonal("0");
    ...
    public String updateInsurance(String formdata) {
        ...
        for (int i = 0, l = staffList.size(); i < l; i++) {
            Map<String, String> staffMap = staffList.get(i);
            InsuranceMiddle middle = get(InsuranceMiddle.class,
                    Restrictions.eq("staffId", staffMap.get("id")),
                    Restrictions.eq("isDelete", 0));
            ...
            if (null != middle) {
                List<InsuranceItems> items = getList(InsuranceItems.class,
                        Restrictions.eq("insuranceMiddleId", middle.getId()),
                        Restrictions.eq("isDelete", 0));
                for (int j = 0, n = items.size(); j < n; j++) {
                    InsuranceItems items2 = items.get(j);
                    jc.put("insuranceId", items2.getIbfId());
                    SalaryInsurance ins = get(SalaryInsurance.class,
                            Restrictions.eq("companyId", staffMap.get("comId")),
                            Restrictions.eq("name", items2.getIbfId()),
                            Restrictions.eq("isDelete", 0));
                    Object comJn = salaryFormulaService.formula(ins.getCompanyFormulaMath(), jc);
                    items2.setPaymentCompany(ceilMath(df, ins.getPayCarryRule(), ins.getNumberAccuracy(), Double.valueOf(comJn.toString())));
                    Object perJn = salaryFormulaService.formula(ins.getPersonalFormulaMath(), jc);
                    items2.setPaymentPersonal(ceilMath(df, ins.getPayCarryRule(), ins.getNumberAccuracy(), Double.valueOf(perJn.toString())));
                    insuranceItemsService.update(items2);
                }
            }
        }
    ...
    public List<Map<String, Object>> getNoChoseStaffListData(String ids, String recordMainId) {
        return salaryInsuranceDao.getNoChoseStaffListData(ids, recordMainId);
    }

    @Override
    public List<Map<String, Object>> getNeedJoinInsuranceItemsData(String ids, String searchData, String recordMainId) {
        return salaryInsuranceDao.getNeedJoinInsuranceItemsData(ids, searchData, recordMainId);
    }
    ...
    public String updateToJfJs(String id, String griddata) throws Exception {
        ...
        for (int j = 0, n = list.size(); j < n; j++) {
            ...
            items.setJfAccount(map.get("jfAccount"));
            items.setBaseCompany(map.get("baseCompany"));
            items.setBasePersonal(map.get("basePersonal"));
            items.setYear(Integer.valueOf(map.get("year")));
            items.setMonth(Integer.valueOf(map.get("month")));
            insuranceItemsService.update(items);
        }
    ...
    public void export(String searchData) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            rowList.add(obj[8]);// 公司
            rowList.add(obj[9]);// 部门
            rowList.add(obj[10]);// 薪酬期间
...
//            Object nextPayBackMonth = obj[21];// 下一条的补缴月
    ...
    public String updateStaffBaseInsurance(String griddata, String column) throws Exception {
        ...
        String[] fuli = column.split(",");
        List<InsuranceItems> items = new ArrayList<InsuranceItems>();
        for (int i = 0, l = gridSalarys.size(); i < l; i++) {
            Map<String, String> map = gridSalarys.get(i);
            InsuranceMiddle middle = get(InsuranceMiddle.class,
                    Restrictions.eq("staffId", map.get("id")),
                    Restrictions.eq("isDelete", 0));
            if (middle == null) {
                throw new Exception(map.get("name") + "保险信息不存在，已经被删除");
            }
            for (int j = 0, m = fuli.length; j < m; j++) {
                InsuranceItems insuranceItems = get(InsuranceItems.class,
                        Restrictions.eq("insuranceMiddleId", middle.getId()),
                        Restrictions.eq("ibfId", fuli[j]),
                        Restrictions.eq("isDelete", 0));
                if (insuranceItems != null) {
                    insuranceItems.setPaymentCompany(map.get("COMJ" + fuli[j]));
                    insuranceItems.setPaymentPersonal(map.get("PERJ" + fuli[j]));
                    items.add(insuranceItems);
                }
            }
        }
        if (items.size() > 0) insuranceItemsService.saveBatch(items);
