...
//				}
				...
				for(int i=1; i<=31; i++){
					...
					flagDay = i;
					String sql="select * from ck_change_apply where month='"+monthCalendar+"' and day='"+String.format("%02d", i)+"' and state=1 and chackno="+checkno;
                    ...
                    Date newdate= sdf2.parse("20"+monthCalendar.substring(0, 2)+"-"
                                  +monthCalendar.substring(2, 4)+"-0"+i+" 00:00:01");
                    Date enddate= sdf2.parse("20"+monthCalendar.substring(0, 2)+"-"
...
//        String depIds = getAllDepIdStrs();
      ...
      String depIds =LingUtil.userinfo().getCid();
        ...
        for (Object[] obj:objList){
        	...
        	for (CkEmpInstitute emp:institutes){
        		if ((obj[2]+"").equals(emp.getInstituteId())){
        			...
        			checkInfo.setCheckDepId(depIds);
        		}
        	}
        ...
        //删除原来考勤的数据
        checkInfosDao.removeByCond(monthDate, depIds);
	}
	...
	public Map<String, Object> getDayDataByCond(Pager pager, String searchData) {
		...
		//获得部门下员工号字符串集合
        String jobNumbers = personnelDao.getJobNumbersByDepIds(depIds).replaceAll("'", "").replaceAll(" ", "");
        String checknos = personnelDao.getChecknoByDepIds(depIds).replaceAll("'", "").replaceAll(" ", "");
        if (!"".equals(jobNumbers)){
    		...
    		if (datas.size() > 0){
    			...
    			//获得员工考勤时间集合
    			List<Map<String, Object>> checkTimes = timerDao.getTimesByJobNumbers(checknos, startTime, endTime);
    			//去员工每日考勤的最早最晚时间
    			if (checkTimes.size() > 0){
    				for (int i=0; i<datas.size(); i++){
    					...
    					for (int j=0; j<checkTimes.size(); j++){
    						...
    						if (empMap.get("checkno").equals(timeMap.get("jobNumber"))){
    							if (flag == 0){
