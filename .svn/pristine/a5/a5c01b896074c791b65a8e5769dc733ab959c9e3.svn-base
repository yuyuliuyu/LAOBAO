...
function remove(b,c,isCommon){
	...
	if(grid.getAllChildNodes){
		...
		for(var i=1;i<l;i++){
			if(rows[i].status==1){
				mini.alert("样本<font color='red'>"+rows[i].sampleName+"</font>已提交，不能删除！")
			}
			if(rows[i].status==2){
				mini.alert("样本<font color='red'>"+rows[i].sampleName+"</font>已审核通过，不能删除！")
			}
			ids=ids+","+rows[i].id;
		}
		deleteok(b,c,ids,grid,isCommon,className);
...
function editWindow(a,b,c,width,height,isCommon){
	...
	var row=rows[0];
	if(row.status==1){
		mini.alert("样本<font color='red'>"+row.sampleName+"</font>已提交，不能编辑！");
		return;
	}
	if(row.status==2){
		mini.alert("样本<font color='red'>"+row.sampleName+"</font>已审核通过，不能编辑！");
		return;
	}
	var w=400;
	var h=300;
...
function commit(a,b){
	...
	if (rows.length > 0) {
		...
		for (var i = 0, l = rows.length; i < l; i++) {
			var r = rows[i];
			if(r.status && (r.status==1 || r.status==2)){
...
function recall(a,b){
	...
	if (rows.length > 0) {
		...
		mini.confirm("确定撤回选中记录？", "确定?", function (action) {
			if (action == "ok") {
				window.parent.loading();
				var url="../"+a+"/"+b+"!recall.action?ids=" +ids;
				$.ajax({
					url: url,
					success: function (text) {
						window.parent.unmask();
