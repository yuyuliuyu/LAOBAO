...
public class HealthDaoImpl extends BaseDaoImpl<Health, String> implements HealthDao{
	...
	public HashMap<String, Object> getListData(Pager pager, String searchData,String depIds) {
		...
		.append(" LEFT JOIN JC_BASIC_INFORMATION BAS ON BAS.ID=JH.PERSON_ID    ")
		...
		if(searchData!=null&&!"".equals(searchData)){
			...
			if (dataMap.get("checkType") != null && !"".equals(dataMap.get("checkType"))){
				sql.append(" and JH.CHECK_TYPE = '"+dataMap.get("checkType")+"'");
			}
		}else {
			sql.append(" AND BAS.DEPART_ID IN("+ depIds+") ");
		}
		sql.append(" ORDER BY JH.CREATEDATE DESC ");
	...
	public List<Object[]> getExceptData(String thePersonId) {
		StringBuilder sql=new StringBuilder()
		.append("  SELECT JH.ID,JH.HEIGHT,JH.BLOOD_TYPE,JH.WEIGHT,JH.HEALTH,JH.CHECK_TYPE,JH.CHECK_DATE,  ")
		.append(" JH.CHECK_RESULT,JH.PROFESSION_TABOO,JH.TABOO_CONTENT,JH.REMARK ")
		.append(" FROM JC_HEALTH JH   ")
		.append("  WHERE JH.PERSON_ID='"+thePersonId+"'")
		;
