...
public class CheckInfosAction extends BaseAction{
    ...
    public String edit() {
        ...
        if(ckCheckInfo.getCheckDepId()!=null){
            CkInfoCommit ckinfo=this.getBeanByCriteria(CkInfoCommit.class, Restrictions.eq("depId", ckCheckInfo.getCheckDepId()));
            if (null != ckinfo) {
                if(ckinfo.getAuditStatus()==2){
    ...
    public String getAllTimesByDay(){
    	...
    	String jobnum=this.getBeanByCriteria(BasicInformation.class, Restrictions.eq("jobNumber", jobNumber)).getCheckNo();
    	allTimes = timerService.getAllListByCond(startTime, endTime, jobnum);
