(function($) {
	...
	$(function() {
		...
		uploader = WebUploader.create({
			...
			server : base + '/../abteilung/send_govern!upLoadFile.action',
			// 只允许选择文件，可选。
			accept : {
				title: 'Images',
				extensions: 'jpg',
		       ...
		       // extensions: 'gif,jpg,jpeg,bmp,png',
		        //mimeTypes: 'image/jpg,image/jpeg,image/png,img/gif,img/bmp'
			},
		...
		uploader.onUploadSuccess = function(file, response) {
			...
			if(result.split("@")[0]=='success'){
				paths.push(result.substring(result.indexOf('@')+1)+'@chargeIds:'+chargeIds);
			}else {
				mini.alert('上传失败！');
				removeFile(file);
			}
