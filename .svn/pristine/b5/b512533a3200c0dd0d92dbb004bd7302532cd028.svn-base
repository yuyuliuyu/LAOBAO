...
<body>
	<form id="form1" method="post">
		<div>
				...
				<tr>
                    ...
                    <td class="list_right_box" width="32%">
                        <input name="jobNumber" id="jobNumber" class="mini-textbox" value="${evection.jobNumber}"
                            required="true" readonly="readonly" requiredErrorText="不能为空" vtype="maxLength:10" width="150px"/>
                    </td>
                ...
                <tr>
                    ...
                    <td class="list_right_box">
                        <input id="startDate" name="startDate" class="mini-datepicker" allowInput="false" 
					      	style="width:150px;" format="yyyy-MM-dd" required="true" onblur="onBlur"
                ...
                <tr>
                    ...
                    <td class="list_right_box">
                        <input field="evectionType" name="evectionType" id="evectionType" required="true" 
							class="mini-combobox" textField="text" valueField="id" emptyText=""  onblur="changeinfo()"
                ...
                <tr>
                	...
                	<td class="list_right_box" colspan="3">
                        <input field="processId" name="processId" id="processId" required="true" 
							...
							width="450px" value="${evection.processId}" url="../system/work_flow!getworkflow.action?type=2"/>
                    </td>
                </tr>
                <tr>
			...
			<table cellpadding="0" cellspacing="0" border="0" class="addtable" width="90%" align="center" id="bj" >  
		                <tr  >
		                	<td class="list_left_box1" >诊断结果</td>
		                	<td class="list_right_box" colspan="3" >
		                		<input name="theEnd" id="theEnd" class="mini-textarea" value="${evection.theEnd}"
		                            required="true" requiredErrorText="不能为空" vtype="maxLength:10" style="width:100%;height:50px;"/>
		                	</td>
		                </tr>
		                <tr>
		                    <td class="list_left_box1" style="width: 20.5%">是否大病：</td>
		                    <td class="list_right_box" style="width: 32%">
		                        <input field="isBigIllness" name="isBigIllness" id="isBigIllness" required="true" 
									class="mini-combobox" textField="text" valueField="id" emptyText="" 
									width="150px" value="${evection.isBigIllness}" url="../template/using.txt"/>
		                    </td>
		                    <td class="list_left_box1" style="width: 20.5%">是否绝症：</td>
		                    <td class="list_right_box" style="width: 32%">
		                        <input field="isFatalIllness" name="isFatalIllness" id="isFatalIllness" required="true" 
									class="mini-combobox" textField="text" valueField="id" emptyText="" 
									width="150px" value="${evection.isFatalIllness}" url="../template/using.txt"/>
		                    </td>
		                </tr>  
			</table> 
		</div>
...
<script type="text/javascript">
	...
	if(mini.get("evectionType").getValue()==2){
	...
	// 申请时间与结束时间焦点离开
	function onBlur(e) {
		var startTime = mini.get("startDate").getValue();
		var endTime = mini.get("endDate").getValue();
		if (startTime && endTime) {
			var sDate = new Date(startTime);
			var eDate = new Date(endTime);
			var number_of_days = (Math.abs(sDate - eDate)) / 86400000;
			mini.get("restDay").setValue(number_of_days + 1);
		}
	}
</script>
</html>
