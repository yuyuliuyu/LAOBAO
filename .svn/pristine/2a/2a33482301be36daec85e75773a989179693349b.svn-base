...
public class ReportAction extends BaseAction{
	...
	public String getRateData(){
		Map<String, Object> result = monthStatisticsService.getRateDataByCond(pager, searchData);//getReportDataByCond(pager, searchData);
        String json = JsonUtil.Encode(result);
        return ajax(Status.success, json);
	}
    ...
    public void exportRateCheckInfo(){
        HttpServletResponse resp = getResponse(); 
        try {
            HSSFWorkbook hwb = monthStatisticsService.exportRateInfo(yearMonth, depIds);
            resp.setContentType("application/x-msdownload");
            resp.setHeader("Content-Disposition", "attachment;filename=\""
                    + new String("员工月度考勤表信息".getBytes("gb2312"), "ISO8859-1")+ ".xls" + "\"");
            OutputStream out = resp.getOutputStream();
