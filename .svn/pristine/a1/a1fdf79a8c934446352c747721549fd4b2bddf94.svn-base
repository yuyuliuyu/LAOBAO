...
<body>
    ...
    <div class="mini-fit">
        <div id="grid1" class="mini-datagrid" style="width:100%;height:100%;" pageSize="20"
            ...
            idField="id" allowResize="false" virtualScroll="true" allowHeaderWrap="true">
        </div>
    </div>
    ...
    <script type="text/javascript">
        ...
        function reJs(type) {
            ...
            mini.confirm("确定要重新计算吗？","提醒", function(action) {
                if (action == "ok") {
                    window.parent.loading();
                    $.ajax({
            ...
            $.ajax({
                ...
                success: function(text) {
                    ...
                    for (var i=0; i< jsonData.length;i++) {
                        var row = jsonData[i];
                        var column = {id: row.id, staffId: row.staffId, jobNumber: row.jobNumber, name: row.name, isAllMonth: row.isAllMonth,
                        		gwName: row.gwName, bzgwName: row.bzgwName, xcgwName: row.xcgwName};
