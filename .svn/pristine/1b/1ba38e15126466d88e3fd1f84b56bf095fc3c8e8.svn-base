...
public class BranchAction extends BaseAction {
    ...
    public String getDataAuth() {
        ...
        return ajax(Status.success, JsonUtil.Encode(branchService.getDataAuth(userId)));
    }
    ...
    public String  getBranchappList(){
        String json = LingUtil.userinfo().getId();
        List<HashMap<String,String>> list = new ArrayList<HashMap<String,String>>();
        String sql = "SELECT B.ID,B.PID,B.FZX,B.FLG, UD.BRANCH_DEP FROM BRANCH B " +
               "INNER JOIN QX_USER_SHOWAUTH UD ON UD.BRANCH_DEP=B.ID AND UD.FLG='0' AND UD.USERID= '"+json+"' " +
               "WHERE B.IS_DELETE=0  order by b.jm ASC";
        List<?> branchList = backendDepService.findBySql(sql);
        for (int i=0,l= branchList.size(); i<l;i++) {
            Object[] b=(Object[]) branchList.get(i);
            HashMap<String, String> map = new HashMap<String, String>();
            map.put("id", b[4]+"");
            map.put("pid","");
            map.put("name", b[2]+"");
            map.put("flg",0+"");
            map.put("branchId",b[4]+"");
    ...
    public String info() {
        ...
        path = pathbase + branch.getImgpath();
        ...
        List<QxDepartment> postlist = getOrderBeanListByCriteria(
                QxDepartment.class, Order.asc("description"),
                Restrictions.eq("barchId", branch.getId()),
        ...
        if (postlist == null || postlist.size() == 0) {
            ...
            for (int i = 0; i < postlist.size(); i++) {
                ...
                try {
                    yhlist = this.getBeanListByCriteria(BasicInformation.class,
                            Restrictions.eq( "departId", postlist.get(i).getId()),
                            Restrictions.eq( "isDelete", 0));
                } catch (Exception e) {
                    yhlist =new ArrayList<BasicInformation>();  
                }
                if(yhlist==null){
                    formations +=postlist.get(i).getName()+"（0）人；";
                }else{
                    formations += postlist.get(i).getName()+"（"+yhlist.size()+"）人；";
                }
            }
        }
        return "info"; 
    ...
    public String getbranch(String id) { 
        String branchname="";
        List<Branch> branchlist=this.getBeanListByCriteria(Branch.class, Restrictions.eq("pid", id), Restrictions.eq("isDelete", 0));
    ...
    public String depLook() {
        ...
        if (department != null) {
            ...
            List<DeptPosition> postlist = postService.getOrderList(
                    DeptPosition.class, Order.asc("positionCode"),
            ...
            if (postlist == null || postlist.size() == 0) {
                ...
                for (int i = 0; i < postlist.size(); i++) {
                    ...
                    try {
                        yhlist = this.getBeanListByCriteria(BasicInformation.class,
                                Restrictions.eq( "specificPostId", postlist.get(i).getId()),
                                Restrictions.eq( "isDelete", 0));
                    } catch (Exception e) {
                        yhlist =new ArrayList<BasicInformation>();  
                    }
                    if(yhlist==null){
                        ...
                        formations += "<a class='linkg' href='javascript:void(0);' onclick='onGwbzLook(\""+id+"\", \""+postlist.get(i).getPositionName()+"\")'>" +
                                postlist.get(i).getPositionName()+"</a>（"+yhlist.size()+"）人；";
                    }
                }
            }
            // 编制集fdepAdd合----待实现
