...
<!-- 							<a class="mini-button" iconCls="icon-remove" onclick="remove()">删除</a> -->
		...
		<div class="mini-fit">
		  <div id="grid" class="mini-datagrid" style="width:100%;height:100%;" allowAlternating="true"
            allowResize="false" url="../check/overtime_record!jsonlist.action?state=${state}" pageSize="20"
	...
	<script type="text/javascript">
		...
		function edit() {
			...
			if (rows.length == 1) {
				$.ajax({
						url : "../check/overtime_record!getAuditStatus.action?id="+rows[0].id,
						type : "post",
        ...
        function view(id){
        	mini.open({
					url : "../check/overtime_record!view.action?id="+id,
					title : "查看加班申请",
					width : 670,
					height :400,
					/* allowResize : false, */
		...
		function appral() {
			...
			if (rows.length == 1) {
				mini.open({
					url : "../check/overtime_sub!add.action?type=3&pid=" + rows[0].id + "&processId=" + rows[0].processId,
					title : "审批页面",
					width : 800,
					height : 450,
					/* allowResize:false, */
