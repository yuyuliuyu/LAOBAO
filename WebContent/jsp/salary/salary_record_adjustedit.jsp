...
<body>
    <div id="form1">
        ...
        <input class="mini-hidden" name="empChangeId" value="${empChangeId}"/>
        <table style="width:100%;" cellpadding="0" cellspacing="0" border="0">
            ...
            <tr>
                ...
                <td class="list_left_box1" valign="top">薪酬岗位：</td>
                <td class="list_right_box">
                    <input id="salaryPost" name="salaryPost" class="mini-lookup" showClose="false" value="${mapInfo.salaryPostId}"text="${mapInfo.salaryPostName}"
    ...
    <div class="mini-fit">
        ...
        <div id="grid2" class="mini-datagrid" style="width:100%;" url="../salary/salary_record!getSalaryGroupValueListData.action?id=${id}" pageSize="20"
            ...
            <div property="columns">
                ...
                <div field="salaryItemsId" displayField="itemName" readOnly="true" width="80" headerAlign="center" align="center">异动薪资项目</div>
    ...
    <script type="text/javascript">
        ...
        function onButtonEditAlert(e) {
            onButtonEdit1(e.sender, "选择薪资组", "../salary/salary_items!salary.action?companyId=${mapInfo.companyId}", 720, 480,"sgName","","");
        }
                        ...
                        if (data) {
                            ...
                            $.ajax({
                                ...
                                success: function(text) {
                                    ...
                                    // 先删除
                                    var editor = grid.getEditorOwnerRow(btnEdit);
                                    // 遍历属于此薪资组的值
                                    var rowData = grid2.getData();
                                    var r = [];
                                    for (var j=0;j < rowData.length;j++) {
                                        if (editor.salaryGroupId == rowData[j].salaryGroupId) {
                                            r.push(rowData[j]);
                                        }
                                    }
                                    ...
                                    for (var j=0;j<parseData.length;j++) {
                                        var rr = parseData[j];
                                        var newRow = {salaryGroupId : rr.groupId, groupName: rr.groupName, salaryItemsId: rr.id, itemName: rr.name, staticValue: $.trim(rr.xs)};
                                        arr.push(newRow);
                                    }
                                    if (arr.length > 0) {
        ...
        function js(rowIndex) {
            ...
            mini.open({
                url : "../salary/salary_record!financial.action?id="+row.salaryItemsId+"&staffId=${id}",
                title : "选择公式",
                width : 540,
                height : 320,
                onload : function() {
