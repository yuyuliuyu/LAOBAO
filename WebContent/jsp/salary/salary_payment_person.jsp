...
<body>
    ...
    <div class="mini-fit">
        <div id="grid" class="mini-datagrid" style="width:100%;height:100%;" pageSize="20"
            url="../salary/salary_payment!getPersonalToAllocationListData.action?id=${id}&ids=${ids}&year=${year}&month=${month}"
    ...
    <script type="text/javascript">
        ...
        function onOKClose() {
            ...
            $.ajax({
                url: "../salary/salary_payment!getCanJoinStaffsListData.action",
                type: "POST",
                data : {
                    ids : ids,
                    year: '${year}',
                ...
                success: function(text) {
                    ...
                    for (var i=0,l = data.length; i< l;i++) {
                        // 判断缴费基数是否存在
                        var row = data[i];
                        if (row.baseCompany == null || row.baseCompany == "") {
                            isFinish = true;
                            mini.alert(row.name + "不存在缴费基数");
                            return;
                        }
                    }
                    if (!isFinish) {
