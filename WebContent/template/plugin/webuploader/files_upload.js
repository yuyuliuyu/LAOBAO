(function($) {
	...
	$(function() {
		...
		uploader = WebUploader.create({
			...
			dnd : '#dndArea',
			paste : '#uploader',
			swf : '${base}template/webuploader/Uploader.swf',
			server : base + '/../action/upload!upLoadFile.action?path='+ path,
			// 只允许选择文件，可选。
			accept : {
				title : 'Docx',
				extensions : 'docx',
...
//			               这两种限制方式均可适用
		...
		function addFile(file) {
			...
			$btns = $('').appendTo($li), $prgress = $li.find('p.progress span'), $wrap = $li
		...
		function updateStatus() {
			...
			if (state === 'ready') {
				text = '选中' + fileCount + '份资料，共'
						+ WebUploader.formatSize(fileSize) + '。';
			} else if (state === 'confirm') {
				stats = uploader.getStats();
				if (stats.uploadFailNum) {
					text = '已成功上传' + stats.successNum + '份资料，'
							+ stats.uploadFailNum + '份资料上传失败'
				}
		...
		function setState(val) {
			...
			case 'finish':
				...
				if (stats.successNum) {
					...
					// 没有成功的图片，重设
					/*
					 * state = 'done'; location.reload(); alert("aaa")
					 */
				}
				break;
			}
		...
		uploader.onUploadSuccess = function(file, response) {
			if (mini.get("modelUrls").getValue()) {
				if (response._raw) {
					mini.get("modelUrls").setValue(
							mini.get("modelUrls").getValue() + ";"
									+ response._raw);
				}
			} else {
				mini.get("modelUrls").setValue(response._raw);
			}

			if (data == null || data == "") {
				data = response._raw;
			} else {
				data = data + "##" + response._raw;
			}
