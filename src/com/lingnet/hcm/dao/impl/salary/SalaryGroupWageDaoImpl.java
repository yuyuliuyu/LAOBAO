...
public class SalaryGroupWageDaoImpl extends BaseDaoImplInit<SalaryGroupWage, String> implements SalaryGroupWageDao {
    ...
    public String getDeployFormulaSql(String id) {
        StringBuilder sql = new StringBuilder();
        sql.append("   SELECT WAT.ID,SF.CNT_MATH,ITEMS.NAME                        ");
        sql.append("   FROM XC_SALARY_GROUP_WAGE WAGE                               ");
        ...
        sql.append("   ON (WAT.SALARY_WAGE_ID = SF.BIND_ID OR WAT.ID = SF.BIND_ID)  ");
        sql.append("   AND SF.IS_DEFAULT = 1                                        ");
        sql.append("   WHERE WAGE.ID = '"+id+"'                                     ");
        sql.append("   AND WAGE.IS_DELETE=0                                         ");
        sql.append("   AND SF.IS_DELETE  = 0                                        ");
        sql.append("   AND WAT.IS_DELETE =0                                         ");
        sql.append("   ORDER BY WAT.SX ASC                                          ");
