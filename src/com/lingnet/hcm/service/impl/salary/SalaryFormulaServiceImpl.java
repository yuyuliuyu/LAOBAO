...
//                            content = sb.toString().replace(name, "formulaUtil." + method);
     ...
     */
    @Override
    public int getFourmulaCount(String content) {
        int level = 0;
        if (StringUtils.isBlank(content)) {
            return level;
        }
        ...
        for (;;) {
            if (!stack.isEmpty()) {
                String item = stack.pop().trim();
                int start = item.indexOf("formulaUtil.");
                if (start > -1) {
                    String name = item.substring(start + 12, item.indexOf("(")==-1? item.length() : item.indexOf("("));
                    if (StringUtils.isBlank(name)) continue;
                    Formula formula = formulaService.get(Formula.class, Restrictions.like("methodEn", name, MatchMode.START),
                            Restrictions.eq("isDelete", 0));
                    if (formula == null) {
                        return -1;
                    } else level += formula.getExecuteIndex();
                }
            } else return level;
        }
    }
