...
public class PostServiceImpl extends BaseServiceImpl<PostDuty, String>
    ...
    public String dutySave(String formdata) throws Exception {
        ...
        if (duty.getId() != null && !"".equals(duty.getId())) {
            PostDuty bean = this.get(PostDuty.class, Restrictions.eq("isDelete", 0),
                    Restrictions.ne("id", duty.getId()),
    ...
    public String treeList() {
        ...
        List<PostDuty> data = this.getOrderList(PostDuty.class, Order.asc("postCode"),
    ...
    public String positionList(String id, Pager pager) {
        ...
        if(id!=null&&!"".equals(id)){
            pager = this.findPagerByOrder(PostPosition.class,pager,Order.asc("positionCode"), Restrictions.eq("dutyId", id), Restrictions.eq("isDelete", 0));
        }else{
            pager = this.findPagerByOrder(PostPosition.class,pager,Order.asc("positionCode"), Restrictions.eq("isDelete", 0));
        }
    ...
    public String positionDeptList(String id, Pager pager) {
        // 根据
        if(id!=null&&!"".equals(id)){
            pager = this.findPagerByOrder(DeptPosition.class,pager,Order.asc("positionCode"), Restrictions.eq("deptId", id), Restrictions.eq("isDelete", 0));
        }else{
            pager = this.findPagerByOrder(DeptPosition.class,pager,Order.asc("positionCode"), Restrictions.eq("isDelete", 0));
        }
        List<HashMap> mapList = ReflectionUtil.getMapList(pager.getResult());
        for(HashMap map :mapList){
            if(map.get("dutyId")!=null&&!"".equals(map.get("dutyId").toString())){
