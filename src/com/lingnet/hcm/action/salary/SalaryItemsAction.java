...
public class SalaryItemsAction extends BaseAction {
     ...
     */
    public String list() {
        QxUserShowauth showDataAuth = getBeanByCriteria(QxUserShowauth.class,
                Restrictions.eq("userid", LingUtil.userinfo().getId()),
                Restrictions.eq("branchDep", LingUtil.userinfo().getCid()),
                Restrictions.eq("flg", "0"));
        if (showDataAuth != null) companyId = "-1";
        return "list";
    }
    ...
    public String getSalaryItemToTypeData() {
        return ajax(Status.success, JsonUtil.Encode(salaryItemsService.getSalaryItemToTypeData(type, depId)));
    }
    ...
    public String getSalaryItemToItemTypeData() {
        return ajax(Status.success, JsonUtil.Encode(salaryItemsService.getSalaryItemToItemTypeData(depId)));
    }
    ...
    public String getPersonalToAllocationListData() {
        return ajax(Status.success, JsonUtil.Encode(salaryItemsService.getPersonalToAllocationListData(ids, depId, recordId)));
    }
    ...
    public String getPersonalGroupListData() {
        return ajax(Status.success, salaryItemsService.getPersonalGroupListData(ids, recordId));
    }
    ...
    public String saveDeploy() {
        try {
            return ajax(Status.success, salaryGroupWageService.saveDeploy(id));
        } catch (Exception e) {
            return ajax(Status.error, e.getMessage());
        }
    }
