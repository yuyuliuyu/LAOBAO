...
public class StandardAction extends BaseAction{
	...
	public String saveOrUpdate(){
		...
		if (!dataMap.isEmpty() && !"".equals(dataMap.get("yearCalendar"))){
			CkStandard standard = standardService.get(CkStandard.class, Restrictions.ne("id", dataMap.get("id")), Restrictions.eq("yearCalendar", dataMap.get("yearCalendar")));
			if (standard != null){
				return ajax(Status.error, "该年月份已存在！");
			}
		}
		try {
			standardService.saveOrUpdateInfo(dataMap);
		} catch (Exception e) {
			e.printStackTrace();
