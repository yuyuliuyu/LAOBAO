...
public class GenerateAction extends BaseAction{
	...
	public String generateMonthTable(){
        ...
        String company = LingUtil.userinfo().getCid();
        List<QxDepartment> departments = getBeanListByCriteria(QxDepartment.class, Restrictions.eq("barchId", company), Restrictions.eq("isDelete", 0));
        for (QxDepartment qxDepartment : departments) {
            List<CkInfoCommit> list1 = this.getBeanListByCriteria(CkInfoCommit.class, Restrictions.ne("auditStatus", 1),
                    Restrictions.eq("depId", qxDepartment.getId()));
            if(list1!=null&&list1.size()>0){
                return ajax(Status.error, qxDepartment.getName() + "本月度有未审核的考勤信息，请全部审核后再生成下月");
            }
        }
//        List<CkCheckInfo> list2 = checkInfosService.getList(CkCheckInfo.class,
//                Restrictions.eq("checkDepId", company),
//                Restrictions.ne("monthCalendar", monthDate));
//        if (list2.size() > 0){
...
//        }
        List<CkCheckInfo> list = checkInfosService.getList(CkCheckInfo.class, Restrictions.eq("monthCalendar", monthDate),
                Restrictions.eq("checkDepId", company), Restrictions.eq("isDelete", 0));
		if (list.size() > 0){
			return ajax(Status.error, "该年月份的班制表已生成，请勿重新生成！");
		}
        try {   
            if (type == 0) {
                List<BasicInformation> baseinfo=this.getBeanListByCriteria(BasicInformation.class,
                        Restrictions.eq("filmId", LingUtil.userinfo().getCid()),
                        Restrictions.isNull("classNo"),
                        Restrictions.lt("onJob", 3));
                if(baseinfo!=null&&baseinfo.size()>0){
                    String message="";
                    for (int i = 0; i < baseinfo.size(); i++) {
                        message+=baseinfo.get(i).getName()+",";
                    }
                    message+="未设置考勤信息或班组信息 <br/> <font color='red' style='font-size: 30px'>确定要生成月度考勤表？</font>";
